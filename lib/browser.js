      ...
      var _ref2 = _asyncToGenerator(function* (browser) {
        ...
        if (browserPages.length > 0) {
          if (usingCustomGetBrowser) {
            debuglog('storing that custom browser provided started with ' + browserPages.length + ' pages');
  ...
  var _ref3 = _asyncToGenerator(function* ({ forceClose, unstableKeepBrowserAlive }) {
    if (browser && (forceClose || !unstableKeepBrowserAlive)) {
      ...
      if (browserPages.length > nrPagesToCloseBrowser) {
        debuglog('keeping browser open as _browserPagesOpen: ' + browserPages.length);
